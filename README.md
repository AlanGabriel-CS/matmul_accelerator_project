...
An AXI-Lite matrix multiplier accelerator, deliberately scaled down from a systolic array to a single-core MAC-based design.

The goal is clean AXI-Lite register-interface integration, memory addressing, and control FSM design, not spatial routing across a grid. Scoped to a solid third-year computer engineering course level and built to run entirely in simulation (Icarus Verilog + cocotb) — no FPGA board or synthesis step.
